...
import ModelApiRoute from "@/routes/ModelApiRoute";
...
class Route {
    public static registerRoutes(app: any): void {
        ...
        app.use("/api/model", ModelApiRoute.route());

        app.use("/api/assets", express.static(join(process.cwd(), Variables.ASSETS_PATH)));
